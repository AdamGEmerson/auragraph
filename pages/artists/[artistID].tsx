...
export const getServerSideProps = async (context : GetServerSidePropsContext ) => {
    ...
    if (typeof artistID === "string") {
        ...
        const artistData = await spotifyApi.artists.get(artistID);
        console.log("Artist Data", artistData)
        const artistTopSongs = await spotifyApi.artists.topTracks(artistID, "US");
...
const ArtistPage = ( { relatedArtists, artistData, artistTopSongs }:{relatedArtists: Response; artistData: Response; artistTopSongs: Response;}) => {
    ...
    const artist = artistData ? JSON.parse(JSON.stringify(artistData.body)) : null;
    const theme = useMantineTheme();
    const preload = false;
    const [topArtists, setTopArtists] = useState<{
        artists: Artist[]
    } | null>(relatedArtists ? JSON.parse(JSON.stringify(relatedArtists.body)) : null);
                  ...
                  <Stack>
                      <Group>
                          ...
                          <HoverCard width={360} shadow="md">
                              ...
                              <HoverCard.Dropdown>
                                  ...
                                  <Text size="sm" mt={'xs'}>
                                      Auragraphs are an attempt to visualize the relationship between your favorite
                                      artists
                                      and their music.
                                  </Text>
                                  <Text size="sm" mt={'xs'}>
                      ...
                      <Group>
                          {artist.genres.map((genre: string) => (
                            <Tooltip color={theme.colors.dark[3]} key={genre} label={genre}
                      ...
                      <Group spacing={'lg'} position={'left'}>
                          ...
                          <Button component={'a'} href={artist.external_urls.spotify} color={'teal'} radius={'lg'}
                                  leftIcon={<IconBrandSpotify/>}>View on Spotify</Button>
                      </Group>
                  </Stack>
              </Group>
